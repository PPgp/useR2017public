...
install.packages(c("foreach", "doParallel", "doRNG", 
                   ...
                   "reshape2", "wpp2017"), dependencies = TRUE)
```
...
* Start a cluster that will be used for multiple tasks:
...
* Pass additional argument to `fun`:
...
* Evaluate a function more times than number of workers (say 20 times): 
    Each function call generates a set of 100,000 random numbers from N(mean=5, sd=1) and returns the average of the set:
...
dim(resmat)
```
Each row in ``resmat`` represents 100 realization of the AR(1) process with parameters corresponding to one row in the ``ar1pars`` data frame.
...
The snowFT package has a solution for the last two bullets above.  The main idea is that there is one RNG stream per task instead of per worker. This makes the process independent of the node on which a task is evaluated, and thus is always reproducible.
...
* For debugging purposes it allows to run your function sequentially while preserving reproducibility (set cluster size to 0):
...
* To run loops in parallel, one needs to register a parallel backend which define the lower level parallel interface to be used.
...
```
    ...
    Note that even though all functions initialize the RNG with the same seed, only `run.snowFT` and `run.foreach` will give reproducible results when called for different number of nodes:
...
* Function that loops over different number of nodes and the various methods and returns the run time:
...
* Compute and plot runtime for different number of trials:
...
2. Which country has the lowest 95%-tile in 2100?
